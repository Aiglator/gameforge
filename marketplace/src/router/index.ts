...
const router = createRouter({
  ...
  routes: [
    ...
    { path: '/confirm-email', name: 'ConfirmEmail', component: () => import('../views/ConfirmEmail.vue') },
    ...
    { path: '/engine',   name: 'Engine',    component: () => import('../views/Engine.vue'),  meta: { fullscreen: true, requiresAuth: true } },
  ],
})

router.beforeEach(async (to) => {
  const auth = useAuthStore()
  if (auth.token && !auth.user) await auth.loadCurrentUser()
